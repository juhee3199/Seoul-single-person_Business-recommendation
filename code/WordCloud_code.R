...
#setwd("C:/Users/ztor7/Desktop/R-proj")
...
text <- readLines("blog_crawling2000_final_4.txt", encoding="UTF-8")
...
head(blog2,30)
temp <- sort(blog2, decreasing = T)[1:30] 
temp

#class(blog2)

# 상위30개에서 필요없는 단어 삭제하기
blog2<-gsub("서울","",blog2)
...
#temp<-sort(blog3,decreasing=T)[1:30]
#temp
...
head(gsubtxt,30)
#class(gsubtxt)  #character

len<-length(gsubtxt)
#gsubtxt<-table(gsubtxt)
...
#공백없애기. blog5
write(unlist(blog4),"blog_crawling_remake.txt")
